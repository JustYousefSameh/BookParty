...
class RoomBloc extends Bloc<RoomEvent, RoomState> {
  ...
  RoomBloc(this._roomRepository, this._authFacade)
      : super(const RoomState.initial()) {
    on<CreateRoom>(
      (event, emit) async {
        final failureOrSuccess = await _roomRepository.createRoom(event.book);

        await failureOrSuccess.fold(
          (l) => null,
          (room) async => await emit.forEach(
            room,
            onData: (data) =>
                data.hasStarted ? RoomStarted(data) : RoomJoined(data),
          ),
        );
      },
    );
